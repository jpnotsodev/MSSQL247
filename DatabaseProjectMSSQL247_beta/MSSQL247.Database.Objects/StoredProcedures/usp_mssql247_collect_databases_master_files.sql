CREATE PROCEDURE [dbo].[usp_mssql247_collect_databases_master_files]
AS
  ;WITH db_master_files AS (
    SELECT dbo.udf_get_server_name() sql_instance, [database_id],
        [file_id], [name], [type], [type_desc], 
        [physical_name], [size], [max_size], [growth],
        [is_media_read_only], [is_sparse], [is_percent_growth]
    FROM sys.master_files
  )
  INSERT INTO mssql247_databases_master_files
